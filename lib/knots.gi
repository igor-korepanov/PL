# Knot diagrams:
# диаграммы узлов задаются следующим образом: каждой двойной точке присваивается имя.
# Затем на узле выбирается произвольное направление и начальная точка. После чего 
# по направлению обхода выписываются последовательно имена вершин, с указанием того сверху
# ли мы проходим эту двойную точку или снизу. Этот список назовем кодом узла. Затем вычислим 
# ориентацию каждой вершины и запишем их в отдельный список.
...
InstallGlobalFunction( KnotInD3,
# ОПИСАНИЕ:
# программа, которая создает погружение узла в трёхмерный диск.
# входные данные: K1 -узел, 
# выходные данные: разбиение 3-диска на шаровые комплексы, с указанием списка ребер по которым проходит узел.
# зависимости: SectionLR
#              PolProduct
#              Diagonal2

# ПРИМЕЧАНИЕ: будем считать, что точка имеет положительную ориентацию, если переход пересекает проход 
# справа налево относительно направления прохода,
#             и точка имеет отрицательную ориентацию, если переход пересекает проход 
# слева направо относительно направления прохода.

function(K1)
     local n,obl,i,K,D2,s,t,D3,rebra,j,m,k,d,knot,otr ;
...
# 3) погружаем узел в диск D3. Каждое ребро на диаграмме узла лежит на соотверствующей
#    2-грани диска D3 (которые когда-то были 1-гранями диска D2).
#    а) создаем списки имен вершин которые соединяются диагоналями(по которым проходит узел).
rebra:=[];
for i in [1..n-1] do
...
InstallGlobalFunction( KnotGroupD3,

# ОПИСАНИЕ:
# программа, которая вырезает узел из 3-диска и вычисляет фундаментальную группу оставшейся части.
# входные данные: К -узел
# выходные данные: группа.
# зависимости: KnotInD3
#              PolMinusPol
#              FundGroup
...
# 1) погружаем узел в 3-диск
D3K:=KnotInD3(K);

# 2) создаем список индексов ребер и вершин которые нужно вырезать из диска.
sp:=rec();
sp.vertices:=[1..Length(D3K.vertices)];
sp.faces:=[];
sp.faces[1]:=D3K.knot;

# 3) вырезаем узел из 3-диска
DMK:=PolMinusPol(D3K,sp);

# 4) считаем фундаментальную группу оставшегося многообразия.
g:=FundGroup(DMK);
